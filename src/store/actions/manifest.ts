...
export const fetchManifest = (url: string, fallbackUrl: string) => async (
  dispatch,
  _getState
) => {
  ...
  await resolver
    ...
    .then((iiifResolver) => {
      ...
      const items: DocumentPage[] = resolver.getPages(
        configuration.customItemProps
      );
      dispatch(loadView(title, metadata, items));
    })
    .catch((e) => {
      const errorCode: string = e.message;
      const errorType: DocumentError = AppConfig.errors.find(
...
export const setManifest = (manifest: Manifesto.IManifest) => (
  dispatch,
  _getState
) => {
...
export const raiseManifestError = (error: DocumentError) => (
  dispatch,
  _getState
) => {
